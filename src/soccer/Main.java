	...
	// 출력
		...
		private static void playerOutput() {
		    ...
		    try {
		        ...
		        // Decorar los objetos SoccerPlayer con la información de goles y asistencias
		        List<SoccerPlayerComponent> decoratedPlayers = new ArrayList<>();
		        for (SoccerPlayer player : list) {
		            SoccerPlayerComponent decoratedPlayer = new SoccerPlayerGoalDecorator(
		                new SoccerPlayerAssistDecorator(player)
		            );
		            decoratedPlayers.add(decoratedPlayer);
		        }
		
		        System.out.println("�̸�\t��\t��\t��ý�Ʈ\t�Ŀ�\t��������Ʈ\t���");
		        for (SoccerPlayerComponent decoratedPlayer : decoratedPlayers) {
